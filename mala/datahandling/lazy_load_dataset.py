                ...
                for x in range(0, self.x_fractions):
                    for y in range(0, self.y_fractions):
                        for z in range(0, self.z_fractions):
                            tmp_tmp = self.input_data[x * self.data_splitting_3d[0]:(x + 1) * self.data_splitting_3d[0],
                                                      y * self.data_splitting_3d[1]:(y + 1) * self.data_splitting_3d[1],
                                                      z * self.data_splitting_3d[2]:(z + 1) * self.data_splitting_3d[2], :]
                            tmp_tmp = np.array(tmp_tmp).transpose([3, 0, 1, 2])
                            tmp.append(tmp_tmp)
                self.input_data = np.array(tmp)
            else:
                self.input_data = np.array([np.array(self.input_data).transpose([3, 0, 1, 2])])
        else:
            self.input_data = \
        ...
        if self.grid_type == "3d":
            if self.data_splitting_3d[0] != 0 and \
                ...
                for x in range(0, self.x_fractions):
                    for y in range(0, self.y_fractions):
                        for z in range(0, self.z_fractions):
                            tmp_tmp = self.output_data[x * self.data_splitting_3d[0]:(x + 1) * self.data_splitting_3d[0],
                                                      y * self.data_splitting_3d[1]:(y + 1) * self.data_splitting_3d[1],
                                                      z * self.data_splitting_3d[2]:(z + 1) * self.data_splitting_3d[2], :]
                            tmp_tmp = np.array(tmp_tmp).transpose([3, 0, 1, 2])
                            tmp.append(tmp_tmp)
                ...
                self.output_data = np.array([np.array(self.output_data).transpose([3, 0, 1, 2])])
        else:
            self.output_data = \
